...
try:
    ...
    if len(golist) > 0:
        ...
        subprocess.call(["youtube-dl", "-o",
                         "%(upload_date)s-%(title)s-%(id)s.%(ext)s", "-x",
    ...
    print("prefs.txt created, change the first line to the URL of the "
          "Periscoper you're trying to download from. Below the first line, "
          "paste in URLs of any broadcasts you've already seen, and don't "
          "want to download!")
    with open('prefs.txt', 'a') as f:
        f.write("https://www.periscope.tv/ABC/\n-----Above this line is the "
                "Periscoper's URL you want to download, and below are any "
                "broadcasts you've already seen-----\n")